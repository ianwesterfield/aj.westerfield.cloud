...
RESTful endpoints for semantic memory:
- POST /save: Store a memory (no classification - caller decides)
- POST /search: Search memories by semantic similarity
- POST /summaries: Get memory summaries for a user

The orchestrator/filter is responsible for:
1. Classifying user intent (recall vs save)
2. Calling /search for recall intents
3. Calling /save for save intents
...
def save_memory(req: SaveRequest) -> Dict[str, Any]:
    ...
    Simply stores the memory with embeddings and extracted facts.
    The caller is responsible for deciding WHEN to save (intent classification).
    
    Returns:
        - status: "saved" or "error"
        - point_id: The ID of the saved point
    ...
    print(f"[/save] user={req.user_id} point_id={point_id}")
