...
class WorkspaceState:
    ...
    def _parse_scan_output(self, output: str, base_path: str) -> None:
        ...
        for line in output.split("\n"):
            ...
            # ls -la format: drwxr-xr-x  4096  Dec 28 20:02  name
            # First char: 'd' = directory, '-' = file
            if len(line) < 10:
                continue
            
            perms = line[:10] if len(line) >= 10 else ""
            is_dir = perms.startswith("d")
            
            # Extract name (last field after date/time)
            # Format: drwxr-xr-x   4096  Dec 28 20:02  name
            parts = line.split()
            if len(parts) >= 5:
                # Name is the last part (may contain spaces, but usually doesn't)
                name = parts[-1].rstrip("/")
