...
def ls_style(
    ...
    *,
    ...
    """
    Returns a directory listing in Unix ls -la style format.
    
    Output format:
    drwxr-xr-x    4096  Dec 28 19:05  filters/
    -rw-r--r--   38618  Dec 29 03:04  aj.filter.py
    """
    ...
    for p in it:
        ...
        entries.append(_Entry(name=display_name, is_dir=is_dir, size=size, mtime=mtime))
    ...
    max_size = max((e.size or 0 for e in shown), default=0)
    size_width = max(len(str(max_size)), 4)
    ...
    for e in shown:
        ...
        # Size (directories show 4096)
        size_str = str(e.size if e.size is not None else 4096).rjust(size_width)
        
        # Date format: Dec 28 19:05
        if e.mtime:
            date_str = e.mtime.strftime("%b %d %H:%M")
        else:
            date_str = "           "
        ...
        lines.append(f"{perms}  {size_str}  {date_str}  {name}")
...
class FileHandler:
    ...
    async def scan_workspace(
        ...
        try:
            ...
            if pretty:
                # Use ls_style for Unix ls -la format output
                formatted = ls_style(
                    validated_path,
                    pattern=pattern,
                    recursive=True,
