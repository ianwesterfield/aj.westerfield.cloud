...
Multi-class intent classifier for AJ conversation routing.
...
# ============================================================================
# Context-Aware Classification - DEPRECATED
# ============================================================================

# REMOVED: All hardcoded pattern lists (AFFIRMATIVE_PATTERNS, TASK_PROPOSAL_PATTERNS)
# Reason: Intent detection must use ML model only, not keyword shortcuts.
#
# Task continuation is now handled by:
# 1. Training data with task continuation examples
# 2. filter.py _detect_task_continuation() calling pragmatics API
# 3. LLM reasoning engine analyzing context
...
def classify_with_context(user_text: str, context: str = "") -> Dict[str, Any]:
    ...
    """
    logger.warning(
        "[classify] DEPRECATED: classify_with_context() called. "
        "Use classify_intent_multiclass() only. No hardcoded keyword shortcuts."
    )
    return classify_intent_multiclass(user_text)
