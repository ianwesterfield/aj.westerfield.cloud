...
def save_merged_model(model, tokenizer, output_path: str):
    ...
    print(f"  Saved! Total size: {sum(f.stat().st_size for f in output_path.rglob('*') if f.is_file()) / 1e9:.2f} GB")
...
def convert_to_gguf(model_path: str, output_path: str, quantization: str = "Q4_K_M"):
    ...
    if not convert_script.exists():
        ...
        print(f"    python convert_hf_to_gguf.py {model_path} --outfile {output_path}.gguf")
        return
...
def main():
    ...
    parser.add_argument("--adapters", nargs="+", help="Adapter paths with weights (path:weight)")
    parser.add_argument("--output", type=str, default="merged-model", help="Output path")
    parser.add_argument("--method", choices=["linear", "ties"], default="linear", help="Merge method")
    parser.add_argument("--ties-density", type=float, default=0.5, help="TIES trim density")
    parser.add_argument("--to-gguf", action="store_true", help="Also convert to GGUF")
    parser.add_argument("--gguf-quant", type=str, default="Q4_K_M", help="GGUF quantization")
