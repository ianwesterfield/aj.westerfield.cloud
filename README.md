...
My agentic-capable AI-assisted plugin for Open-WebUI. Handles semantic memory based on intent classification, workspace ops, and surgical file editing.

## What It Does

AJ sits between your LLM and your workspace as an intelligent filter.

1. **Classifies Intent** — Figures out if you're chatting casually, saving info, recalling memories, or asking for a task.
2. **Manages Memory** — Stores facts, docs, and image descriptions; pulls relevant context automatically in later sessions
...
flowchart TD
    ...
    D -->|recall/save/casual| F["Memory<br/>Search"]
    E --> G["Executor"]
    G --> H["Updated Context"]
    F --> I["Updated Context"]
    H --> J["LLM Response"]
...
| ------------------ | ----- | -------------------------------------- |
| `memory_api`       | 8000  | Semantic memory (save/search)          |
...
The AJ filter runs inside Open-WebUI (stored in DB, not mounted). To sync changes:
...
├── filters/
│   └── aj.filter.py          # Open-WebUI filter (intent routing, memory)
├── layers/
│   ├── memory/               # Semantic memory (port 8000)
│   │   ├── api/memory.py     # /save, /search endpoints
│   │   └── services/         # Embedder, Qdrant, Fact Extractor
...
Invoke-RestMethod -Uri 'http://localhost:8005/api/execute/tool' -Method Post `
  ...
  -Body (@{
    tool='append_to_file'
    params=@{path='/workspace/test.txt'; content='Hello from AJ'}
